...
/**
 * Webwork duplicate namespace checker
 *
 * @author Jaehyeon Nam (dotoli21@gmail.com)
 * @since 2010. 5. 5.
 */
public class DuplicateNamespaceWebworkCheckerImpl implements WebworkChecker {
	...
	public List<WebworkConfigurationError> check(List<WebworkPackage> webworkPackages) {
		...
		for (WebworkPackage webworkPackage : webworkPackages) {
			if (StringUtils.isNotBlank(webworkPackage.getNamespace())) {
				if (packageNameSet.contains(webworkPackage.getNamespace())) {
					final WebworkConfigurationError error = new WebworkConfigurationError();
					error.setErrorType(WebworkConfigurationErrorType.DUPLICATE_NAMESPACE);
					...
					packageNameSet.add(webworkPackage.getNamespace());
				}
			}
		}
